# Assumptions – Data Quality Analysis: Article Master Data

**Project:** dq_artikelstammdaten  
**Author:** Anna Sommani  

All assumptions are documented and explained here transparently. Where an assumption affects the outcome of a specific KPI or check, this is noted explicitly.
...
**Rate denominator:** Completeness rate is computed as `1 - (missing / n_total)` per field, and the overall rate is computed over all mandatory fields combined: `1 - (total_missing / (n_rows x n_fields))`. This follows the Microsoft Purview convention: `score = passed / (passed + failed + empty)`, meaning missing values reduce the completeness score rather than being excluded.
...
**Duplicate counting:** Duplicates are identified using pandas `duplicated()` after stripping empty/whitespace strings (via `replace(r"^\s*$", pd.NA)`). The `eindeutig_rate` reflects the number of distinct non-null values relative to total rows, not relative to non-null rows.
...
This is a deliberate design choice: conflicts in non-dimension fields (e.g., different Disponenten for the same Artikel+Werk key) are treated as a separate data quality issue (see Werksdaten Konflikte check) and are not propagated into the consistency KPI. The consistency KPI answers only: 'Of the comparable pairs, how many agree?'
